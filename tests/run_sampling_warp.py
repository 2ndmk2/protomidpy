...
parser.add_argument("--config", default=str(TESTS_DIR / "paras" / "warp_config.dat"))
parser.add_argument("--initial_para", default=str(TESTS_DIR / "paras" / "warp_paradic.dat"))
parser.add_argument("--prior", default=str(TESTS_DIR / "paras" / "warp_prior.dat"))
...
if __name__ == "__main__":
    ...
    with Pool(processes=n_process) as pool:
        main.sample_mcmc_full(
            ...
            "RBF",
            -1000,
            para_dic_for_mcmc_config["Nwalker"],
            para_dic_for_mcmc_config["Nchain"],
            para_dic_for_mcmc_prior,
            para_dic_for_mcmc_initial,
            header_name_for_file=header_name_for_file,
            out_dir=para_dic_for_mcmc_config["Out_folder"],
            n_bin_log=para_dic_for_mcmc_config["Nbin"],
            nrad=para_dic_for_mcmc_config["Nrad"],
            dpix=para_dic_for_mcmc_config["Dpix"] * ARCSEC_TO_RAD,
            q_min_max_bin=[para_dic_for_mcmc_config["Qmin"], para_dic_for_mcmc_config["Qmax"]],
            pool=pool,
        )
