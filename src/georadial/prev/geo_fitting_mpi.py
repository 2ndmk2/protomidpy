...
def obs_model_comparison(I_model, u_d, v_d, cosi, pa, delta_x, delta_y, d_data, R_out, N, dpix):
    cos_pa = np.cos(pa)
    sin_pa = np.sin(pa)
...
def make_initial_geo(log10_alpha_min, log10_alpha_max, gamma_min, gamma_max, delta_pos ,nwalker,  x_est = None, y_est = None):
    """ Generate random walkers for emcee
...
def main(N_RAD=30, D_PIX= None, DO_TEST_CALC=True, DO_MCMC=True, TAKE_RANDOM_SAMPLE = True, MCMC_RUN=10000, NWALKER=32, \
    ...
    LOGALPHAMIN=-13, LOGALPHAMAX=-4, D_OFFSET=0.9 * ARCSEC_TO_RAD, NUM_BIN_FOR_DATA_LINEAR=200, NUM_BIN_FOR_DATA_LOG=200, MIN_BIN = 5,  BIN_DATA_REPLACE =True, vis_folder = "./vis_data", outdir ="./out_dir", 
    target_object = None, NUMBER_OF_TARGETS = 1, Q_MAX =None, PA=0, COSI=0, N_process = 4, pool = None):
    ...
    for file in files:
        ...
        if Q_MAX is not None:
            ...
            count_grid_1d = count_grid_1d[mask]
        ...
        if DO_TEST_CALC:
            ...
            #"""
            alpha_test, gamma_test = 10**(-5.8), 0.1* ARCSEC_TO_RAD
            pa_test = 85.7 * np.pi/180 
            inc_test = np.cos(35.2 * np.pi/180)
            delta_x_test = 0. * ARCSEC_TO_RAD
            delta_y_test = 0. * ARCSEC_TO_RAD


            theta_test = (gamma_test, alpha_test, inc_test, pa_test, delta_x_test, delta_y_test)
            #print(theta_test)
            sample_one, H_mat= sample_radial_profile(r_dist, theta_test, u_grid_1d, v_grid_1d, MAXRAD, N_RAD, D_PIX, d_data, sigma_mat_1d)
            ...
            H_mat, q_dist_forp, d_real_mod_forp, d_imag_mod_forp, vis_model_real, vis_model_imag, u_mod, v_mod= obs_model_comparison(sample_one, u_grid_1d, v_grid_1d, theta_test[2], theta_test[3], theta_test[4], \
                    theta_test[5], d_data, MAXRAD, N_RAD, D_PIX) 
            chi_sq = np.sum(((d_real_mod_forp-vis_model_real)/sigma_mat_1d_for_plot_real)**2 \
                + ((d_imag_mod_forp-vis_model_imag)/sigma_mat_1d_for_plot_imag)**2) 
            ...
            mask = masking_data(H_mat, sample_one, d_real_mod_forp, d_imag_mod_forp,  sigma_mat_1d_for_plot_real,\
              sigma_mat_1d_for_plot_imag, threshold = 5)

            print(chi_sq, len(d_data))

            #print(len(u_mod[mask]))
            plt.scatter(u_mod, d_real_mod_forp, color="k", s = 10)
            plt.errorbar(u_mod, d_real_mod_forp, yerr =  sigma_mat_1d_for_plot_real, ls = "None",  color="k")
            plt.scatter(u_mod[mask], d_real_mod_forp[mask], color="b", s = 10)
            plt.errorbar(u_mod[mask], d_real_mod_forp[mask], yerr =  sigma_mat_1d_for_plot_real[mask], ls = "None",  color="b")
        ...
        if DO_MCMC:
            ...
            delta_pre_fitting = np.array([0,0])

            print(delta_pre_fitting/ARCSEC_TO_RAD)
            initial_for_mcmc = make_initial_geo(LOGALPHAMIN, LOGALPHAMAX, MIN_RSCALE, MAX_RSCALE, D_OFFSET*1, NWALKER, delta_pre_fitting[0], delta_pre_fitting[1])
            print(np.shape(initial_for_mcmc))
            if not TAKE_RANDOM_SAMPLE:
                n_iter = 30
                initial_for_minimize = make_initial_geo(LOGALPHAMIN, LOGALPHAMAX, MIN_RSCALE, MAX_RSCALE, D_OFFSET*1, n_iter, delta_pre_fitting[0], delta_pre_fitting[1])
                best_parameters = []
                best_chi = []
        ...
        if DO_SAMPLING:
            ...
            mask = (sample[:,0]>0. * ARCSEC_TO_RAD)# * (post>-1e99) * ((post<(np.min(post[post>-1e99]) * 1.05)))
            #mask = (sample[:,0]>0. * ARCSEC_TO_RAD) * (chi_sq>0) * (post < np.min(post) + 100)
            #mask = (sample[:,0]>0. * ARCSEC_TO_RAD) * (chi_sq>0) 
            sample = sample[mask]
            n_chain = len(sample)
            burn_out = n_chain - 10000
            plot_out_name_npz = os.path.join(outdir, header_name_for_file +"_mcmc.pdf")
            sample_for_plot_ = samples_for_plot(sample[burn_out:])
